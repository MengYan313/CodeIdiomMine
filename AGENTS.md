...
- 冻结 DBSCAN 与阶段3之间必须按仓库执行保守簇归并。C++ 词法等价中心直接
  归并；AST同构且高相似的中心只有在全部差异均为已声明局部变量一致换名时才
  归并。禁止跨仓归并；合并后保留全部成员与 `infos`，以全部原始 embedding
  重算质心并选择最近真实代码。七列簇表 Schema 不变，来源 label 和理由保存在
  顶层 `clustering_metadata.postprocessing`；不得覆盖冻结 DBSCAN 产物。
- 习语判断输出 `idiom-judgment.pkl`：按簇保存 `accepted` 和 `rejected`，离线规则预检另存 `pending_llm`；记录规则证据、保守抽象提案、完整 `member_codes`/`source_infos`、原始成员数/词法变体数/文件数/源码位置数、经哈希验证但不发送给阶段3 LLM 的代表函数/区域上下文证据、只含代表代码与词法去重真实变体的 LLM 输入、`is_idiom` 与 `abstract/keep` 决策、实际批准集合、受控通用类型或 `仓库特有习语` 的分类、语义/类型/异味理由、最终裁决理由、语义/复用价值业务评分，以及与业务评分分离的共享代码异味审查输入、分类发现和独立门禁。拒绝抽象只表示保持代表代码不变，不得据此拒绝本来合格的习语；`agent_trace` 记录各 Agent 的逻辑尝试数、技术失败类型和回退动作。
- 习语合成输出 `idiom-synthesis.pkl`：Schema v9 正式消费习语判断的已接受产物，是只记录合成尝试及成功结果、不复制未合成阶段3习语的 `synthesis_delta`；使用完整 `source_infos` 按成员 `project + file + function_extent` 的交集发现同区域共现，同一簇在一个区域内只形成一个区域绑定候选，但可参与多个真实共现区域。顶层 `source_judgments` 携带来源阶段3理由与类型，`matched_source_infos`/`matched_occurrences` 保存本次合成实际使用的局部成员位置，完整 `source_infos` 继续保存簇级支持证据；`region_planning` 保存每区域一次批量规划、计划校验和去重证据，每个合法计划以稳定 `combination_key` 独立记录。产物同时保存自动同区域上下文证据、单项合成计划、组装证据、对当前合成结果重新执行的 `is_idiom` 与习语类型判断、规划/组装/质量/类型/异味理由、最终裁决理由、质量复审业务分、与阶段3同合同但独立执行的代码异味审查输入/分类发现/门禁、确定性检查、`agent_trace` 和 `synthesis_trace`。阶段2适配只保留合同和后备逻辑验证，不进入正式 CLI 或实际实验执行。
...
习语判断直接使用 `autogen_core`：确定性规则先基于完整成员拒绝合同无效和确定性低价值簇，抽象规则只对至少3个结构对齐实例中至少3个不同取值、覆盖率至少60%的局部变量或低语义字面量提出候选，调用名、类型、控制条件、返回值和哨兵值不得仅因变化而抽象；随后语义/抽象 Agent 只读取代表代码、按 C++ 词法 token 去重的其他真实变体、四项簇统计，以及规则证据和全部提案，显式返回 `is_idiom`、非空判断理由、`abstract` 或 `keep`，且只能批准规则候选。完整成员和 `source_infos` 不进入阶段3 LLM 请求，用于本地规则、支持度、审计、评价、实例归类，并在阶段4用于成员共现区域发现。最终有效性判断使用 `src.idiom_judgment.idiom_taxonomy` 的受控目录；能精确对应时输出通用类型，不能对应但仍通过全部门禁时输出 `仓库特有习语`，不得强行匹配。正式运行使用严格上下文门禁，路径、范围或 `source_sha256` 失败时零调用拒绝；验证后的代表上下文不进入阶段3 LLM 请求。有效响应中的 `keep` 或无提案均保留代表代码不变，候选仍按语义/复用价值和异味门禁判断是否进入阶段4；整份语义/抽象响应失败时保留原代码作为审计证据，但业务分安全降级并拒绝。规则、语义和复用价值形成业务分，异味不进入业务分；分类风险达到冻结阈值或异味分析失败时由独立门禁直接拒绝。所有承担有效性、选择或生成判断的 Agent 都必须输出非空理由。

习语合成使用独立 runtime，正式接受习语判断已接受产物；阶段2簇到同一内部候选 Schema 的适配和严格阈值只作不启动正式 Agent 的合同验证。编排层以完整簇成员位置建立区域倒排索引，在任何 LLM 调用前自动加载并验证成员共同出现的函数/区域上下文；候选自身字节范围用于局部代码和稳定源码顺序，不要求不同习语片段的字节区间相交。合成规划、代码组装、质量/有效性/类型复审和与阶段3相同的共享异味审查通过路由消息协作。只能合成同仓库同一成员共现区域内至少两个具有数据、控制、生命周期、异常处理或稳定顺序关系的不同习语，不得扩大到跨区域。规划 Agent 的设置目的就是避免对区域内候选执行指数级全子集枚举；“发现所有组合”只表示一次性返回显式上限内所有具有明确语义关系且值得尝试的组合，不表示枚举全部数学子集。规划 Agent 每区域只调用一次并返回有界 `plans`；编排层负责索引排序、去重、范围与非空字段校验，按规范化候选集合生成稳定组合键，再逐计划独立组装和复审。一个计划失败不得取消同一区域其他计划，不得改为每次只返回一组并由模型控制循环停止。候选或计划超过显式上限时不得静默截断。核心确定性门禁只检查上下文合同、Tree-sitter 语法和新增调用目标。质量复审必须重新判断当前合成结果是否属于习语并重新分类，不能直接继承阶段3类型；质量分单独决定业务质量，异味按同一分类和阈值重新审查当前合成代码并可独立否决。阶段3和阶段4的异味过滤必须以分层人工审计报告总体、分阶段和逐类别准确性。
