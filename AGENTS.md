...
习语判断直接使用 `autogen_core`：确定性规则先基于完整成员拒绝合同无效和确定性低价值簇，抽象规则只对至少3个结构对齐实例中至少3个不同取值、覆盖率至少60%的局部变量或低语义字面量提出候选，调用名、类型、控制条件、返回值和哨兵值不得仅因变化而抽象；随后语义/抽象 Agent 只读取代表代码、按 C++ 词法 token 去重的其他真实变体、四项簇统计，以及规则证据和全部提案，显式返回 `is_idiom`、非空判断理由、`abstract` 或 `keep`，且只能批准规则候选。完整成员和 `source_infos` 只用于本地规则、支持度、审计、评价与实例归类。最终有效性判断使用 `src.idiom_judgment.idiom_taxonomy` 的受控目录；能精确对应时输出通用类型，不能对应但仍通过全部门禁时输出 `仓库特有习语`，不得强行匹配。正式运行使用严格上下文门禁，路径、范围或 `source_sha256` 失败时零调用拒绝；验证后的代表上下文不进入阶段3 LLM 请求。有效响应中的 `keep` 或无提案均保留代表代码不变，候选仍按语义/复用价值和异味门禁判断是否进入阶段4；整份语义/抽象响应失败时保留原代码作为审计证据，但业务分安全降级并拒绝。规则、语义和复用价值形成业务分，异味不进入业务分；分类风险达到冻结阈值或异味分析失败时由独立门禁直接拒绝。所有承担有效性、选择或生成判断的 Agent 都必须输出非空理由。
