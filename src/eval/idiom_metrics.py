...
def evaluate_language(
    ...
    对每个项目计算 IC, ISP, F1, avg_idiom_size，最后汇总到 result/{lang}/eval.json

    Args:
        language: 语言标识，如 cpp
        idiom_dir: 习语目录，如 result/cpp
        dataset_path: 数据集路径，如 output/cpp/dataset.pkl
        output_path: 汇总结果输出路径，如 result/cpp/eval.json
    """
    ...
    if not project_results:
        summary = {"language": language, "projects": [], "summary": {}}
    else:
...
def run_evaluation(
    ...
    Args:
        language: 语言标识
        idiom_dir: 习语目录，默认 result/{language}
        dataset_path: 数据集路径，默认 output/{language}/dataset.pkl
        output_path: 输出路径，默认 result/{language}/eval.json
    """
    project_root = Path(__file__).parent.parent.parent
    idiom_dir = idiom_dir or str(project_root / "result" / language)
    dataset_path = dataset_path or str(project_root / "output" / language / "dataset.pkl")
    output_path = output_path or str(project_root / "result" / language / "eval.json")
...
def main():
    ...
    parser.add_argument(
        "--idiom-dir", "-i",
        default=None,
        help="习语目录，默认 result/{language}",
    )
    parser.add_argument(
        "--dataset", "-d",
        default=None,
        help="数据集路径，默认 output/{language}/dataset.pkl",
    )
    parser.add_argument(
        "--output", "-o",
        default=None,
        help="输出路径，默认 result/{language}/eval.json",
    )
...
# python -m src.eval.idiom_metrics -l cpp -i result/cpp -d output/cpp/dataset.pkl -o result/cpp/eval.json
