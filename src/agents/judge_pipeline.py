...
class CodeIdiomPipeline:
    ...
    def __init__(self, model: Optional[str] = None, quiet: bool = False):
        """
        初始化判定流水线
    ...
    async def initialize(self):
        ...
        await register_agent(
            self.runtime,
            "semantic_agent",
            lambda: SemanticClarityAgent(self.model_client)
        )
        if not self._quiet:
        ...
        await register_agent(
            self.runtime,
            "syntax_agent",
            lambda: SyntaxLogicAgent(self.model_client)
        )
        if not self._quiet:
        ...
        await register_agent(
            self.runtime,
            "judge_agent",
            lambda: IdiomJudgeAgent(self.model_client)
        )
        if not self._quiet:
