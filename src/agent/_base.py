...
1. ``load_project_env()``      —— 加载仓库根目录 ``.env``（OPENAI_API_KEY / OPENAI_BASE_URL）。
...
def load_project_env() -> None:
    """加载仓库根目录下的 .env（幂等；未安装 python-dotenv 时静默跳过）。"""
    global _ENV_LOADED
    if _ENV_LOADED:
        return
    _ENV_LOADED = True
...
class JsonLLMAgent(RoutedAgent):
    """
...
def run_agent_selftest(
    ...
    async def _run() -> None:
        ...
        await runtime.register_factory(
            agent_name, lambda: agent_factory(model_client)
        )
        runtime.start()
        try:
