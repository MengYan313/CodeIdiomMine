...
def load_pkl(file_path: str, limit: int = -1) -> pd.DataFrame:
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if limit != -1:
            ...
            print(f"限制加载前 {limit} 行，当前形状: {data.shape}")
...
def dense_clustering(embeddings: List[torch.Tensor]) -> Tuple[float, int]:
    ...
    print(f"优化结果:")
    print(f"  最佳 eps: {best_eps:.4f}")
    print(f"  最佳 min_samples: {best_min_samples}")
...
def perform_dbscan_clustering(
    ...
    # 如果需要优化参数
    if optimize_params:
        print(f"优化项目 {pro_name} 的聚类参数...")
        eps, min_samples = dense_clustering(pro_emb)
    
    # 执行 DBSCAN 聚类
    print(f"执行 DBSCAN 聚类 (eps={eps:.4f}, min_samples={min_samples})...")
    ...
    print(f"聚类结果: {num_clusters} 个簇, {num_noise} 个噪声点")
    sys.stdout.flush()
    ...
    for label in unique_labels:
        ...
        if closest_point_str:
            print(f"  簇 {label} (大小: {len(points_in_cluster)}): {repr(closest_point_str[:100])}")
            sys.stdout.flush()
...
def clustering(
    ...
    Returns:
        聚类结果 DataFrame
    """
    cluster_labels, cluster_data = perform_dbscan_clustering(
...
def process_projects(
    ...
    for i, pro_name in enumerate(pros_name):
        print(f"\n{'='*60}")
        print(f"处理项目 [{i+1}/{len(pros_name)}]: {pro_name}")
        print(f"{'='*60}")
        sys.stdout.flush()
        ...
        if any("ToBeDetermined" in src for src in pro_src):
            print(f"跳过项目 {pro_name}（包含无效代码片段）")
            continue
        ...
        for idx, row in top100_clusters.head(10).iterrows():
            ...
            print(f"    簇 {row['label']} (大小: {row['cluster_size']}): {snippet}...")
...
def save_cluster_results(cluster_results: List[Dict], output_path: str):
    ...
    print(f"共 {len(cluster_results)} 个项目的聚类结果")
...
def run_clustering(
    ...
    print(f"加载嵌入数据: {embedding_file}")
    data = load_pkl(embedding_file)
    
    print("\n开始聚类...")
    cluster_results = process_projects(data, eps, min_samples, optimize_params)
