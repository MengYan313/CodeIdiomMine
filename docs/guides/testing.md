...
阶段2合同适配与阶段3正式合成输入、完整簇成员同区域共现、区域实际成员与源码
顺序绑定、自动验证共现区域上下文、
...
正式输入、全成员位置倒排分组、单簇重复成员去重、同一簇参与多个共现区域、
局部代码与源码顺序进入 Prompt、每区域一次批量规划、计划索引规范化与稳定键、
重复/越界/单候选计划拒绝、多计划逐项执行与失败隔离、自动上下文与零调用失败、共享异味分类、独立
过滤门禁、分层事后审计与逐类别指标、跨区域禁止分组、合成增量产物、新增调用拒绝、
...
- 阶段4同区域候选超过上限时保留整组成员共现证据而不静默截断，四个 Agent 均不运行；
- `is_idiom=false`、未知目录编号、矛盾分类、非有限置信度或空判断理由都不能
  进入已接受产物；
- artifact 的 `agent_trace` 与 `technical_failure_count` 能复现技术失败。

阶段4批量多计划的确定性离线验收固定检查：规划 Agent 每区域只调用一次；同一
响应中的多个合法计划分别进入组装和复审；索引顺序不同但候选集合相同的计划只
执行一次；共享候选但集合不同的计划分别保留；越界、重复、少于两个候选或缺少
必填理由的计划被编排层拒绝；一个计划失败不影响同区域其他计划；响应超过显式
计划上限时不截断也不执行，测试不得通过枚举全部数学子集制造调用量。
...
区域至多调用规划 Agent 一次，并为每个合法计划调用一次组装及并行的质量/异味
复审。上下文在调用前由
编排层自动读取和校验，失败时零调用拒绝。每个 Agent 单次请求最多等待120秒，
每次逻辑尝试在 JSON
首次失败时可能增加1次修复请求；修复耗尽或请求异常后最多再执行1次逻辑尝试，
故单 Agent 最多4次端点请求。阶段3理论端点上限为每簇8次；阶段4默认每区域最多
8个计划，对应25次逻辑调用和100次端点请求。规划或单项组装失败时会跳过相应
下游，实际失败路径上限随之降低。执行前必须按端点请求
