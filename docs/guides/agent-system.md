...
阶段2 clusters-merged.pkl
 ...
 idiom_synthesis 阶段3正式输入
   → 完整 source_infos 按成员 project/path/function_extent 建立区域倒排索引
   → 自动读取并验证成员共现区域上下文
   → 合成规划 Agent
   → 代码组装 Agent
...
完整成员代码和 `source_infos` 不进入阶段3 LLM 请求；它们由本地规则、支持度、
审计、评价和实例归类使用，并在阶段4用于成员共现区域发现。
...
两者归一化为 `IdiomCandidate`。编排层遍历完整 `source_infos`，按成员
`project + file + function_extent` 建立区域倒排索引；至少两个不同习语簇在
同一区域各有成员时才形成候选组。同一簇在一个区域内只形成一个区域绑定候选，
但可以参与多个真实共现区域。候选自身 `extent` 与 `start_byte/end_byte` 用于
局部代码和稳定源码顺序，不要求不同习语片段的字节区间相交，也不因 embedding
相似就跨区域或跨仓强行合并。阶段4 artifact 明确标记为
...
1. **确定性上下文加载器**：在调用 LLM 前，根据当前区域的成员文件、外层范围和
   `source_sha256` 自动读取同一共现区域；成员身份、哈希或上下文校验失败时直接
   拒绝。
2. **合成规划 Agent**：每区域只调用一次，同时审查全部候选并批量返回有界
   `plans`。每项至少选择两个真正互补的习语，分别声明 `relation_kind`、合成目标、
   顺序约束、预期质量增益和理由；它同时接收簇级模板、当前区域
   `matched_occurrences` 及精确源码顺序，找不到关系时返回空数组和非空总理由。
3. **代码组装 Agent**：严格按计划组装，只能使用选定习语的模板、当前区域实际
   成员代码和已验证上下文，并说明实际组装依据。
...
规划 Agent 是组合空间的语义剪枝器，而不是全子集枚举器。区域内 \(n\) 个候选
共有 \(2^n-n-1\) 个至少含两个候选的数学子集；逐一组装和复审会造成不可接受的
调用量。多计划实现因此固定为每区域一次批量规划：Agent 在显式计划上限内
返回全部具有明确依赖或顺序关系且值得尝试的 `plans`，不返回缺乏语义关系的任意
子集。编排层对每个计划的索引排序、范围和唯一组合键进行确定性校验与去重，再逐
计划独立调用组装和复审。不得采用“每轮由 Agent 返回一组，直到 Agent 自称没有
更多组合”的开放循环，因为该停止条件无法保证覆盖、成本或可复现性。默认
`max_plans_per_region=8`；超过上限不截断也不执行。

上下文加载、规划和组装顺序执行；质量与异味复审并行。各 Agent 都使用强类型
消息、显式 JSON Schema、原生 JSON mode、严格解析和单次修复；请求异常或修复
耗尽时最多再执行1次完整逻辑尝试。Schema v9 以顶层 `source_judgments` 保存
来源阶段3理由和类型，以 `matched_source_infos`、`matched_occurrences`、
`region_identity` 和 `source_order_candidate_ids` 保存本次实际使用的成员共现
证据；完整 `source_infos` 继续保存簇级支持证据。`region_planning` 保存区域级
规划总理由、调用状态、上限及非法/重复计划摘要，每个计划记录稳定
`combination_key`。当前合成结果另存单项规划、组装、质量、类型、异味理由及
最终裁决理由。

### 3.3 上下文与确定性门禁

`--source-root` 是正式 CLI 的必需输入。编排层读取当前成员共现区域指向的源码
行区间，拒绝成员区域不一致、路径越界、仓库身份不一致、文件缺失、内容 SHA
不一致、超过300行或12,000字符的上下文。任一检查失败时结果直接 `rejected`，
且不会产生 LLM 调用。
...
Agent 的端点请求上限为4。有效业务结果中的 `keep`、低分、无异味或带非空总理由
的空 `plans` 不属于技术失败，不会重试。日志只记录 Agent 名称、
...
| 规划 Agent 两次失败 | 无计划并拒绝当前区域 | 该区域全部下游 Agent |
| 规划正常返回空 `plans` | 保存总理由并拒绝当前区域 | 该区域全部下游 Agent |
| 规划返回越界、少于两个候选、空字段或重复集合 | 编排层拒绝相应计划 | 仅该非法/重复计划 |
| 规划响应超过计划上限 | 不截断并拒绝当前区域 | 该区域全部下游 Agent |
| 组装 Agent 两次失败或空输出 | 空结果并拒绝当前计划 | 当前计划的两个复审 Agent |
| 语法失败或新增调用越界 | 确定性拒绝当前计划 | 当前计划的两个复审 Agent |
| 质量复审两次失败 | `quality_score=0` | 异味并行分支仍独立完成，最终拒绝 |
| 异味复审两次失败 | 独立异味门禁拒绝 | 质量并行分支仍独立完成 |
| 未预料的计划级编排异常 | 写入 `unexpected_plan_orchestration_error` | `skip_plan`，继续同区域其他计划 |

阶段4 artifact Schema v9 使用同一 `agent_trace` 合同，并对当前合成结果保存新的
类型和完整理由链。取消信号和进程中断不会被转换为业务拒绝；只有当前簇/区域内的
普通技术异常会被隔离。

两个 CLI 都可选择 SQLite `--checkpoint`，每完成一个簇或区域立即提交结果；中断后
...
关联闭环融合每区域固定至多1次规划逻辑调用，并为每个合法计划至多发起组装、质量
复审和共享异味审查3次逻辑调用。默认8个计划时每区域最多25次逻辑调用、100次端点
请求；规划或单项组装耗尽时会跳过相应下游，避免继续付费调用。
