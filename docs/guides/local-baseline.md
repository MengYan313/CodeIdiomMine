...
输入范围对当时的三个源码快照应用既有 `FileScanner` 合同：Envoy 2,924 个文件、qBittorrent 465 个文件、React Native 1,415 个文件，共 4,804 个。该运行是历史证据，不是当前 23 项正式数据集的启动说明。
...
- 当前正式聚类统一使用 DBSCAN，并对每个新仓库执行同一套无监督自动调参规则；
  ...
  `--optimize` 入口仍保留。
- 当前阶段3已由 `src.idiom_judgment` 独立实现单簇合同/低价值规则、高频低语义差异的保守抽象提案、基于代表代码/词法去重变体/四项统计与规则初判的 LLM `abstract/keep` 决策、语义/复用价值与代码异味独立审查，以及固定门槛裁决。完整成员保留在本地规则与产物中，不进入提示词。它不执行全面 AST 反统一；拒绝抽象时保留原代码，也尚未在23仓正式语料上运行付费 LLM 验证或人工质量核验。
- 当前阶段4已由 `src.idiom_synthesis` 独立实现阶段3正式输入、阶段2非执行合同
  适配、完整簇成员位置的同区域共现发现、当前区域实际成员与源码顺序绑定、自动
  验证共现区域上下文、关系规划、代码组装、质量复审、共享异味审查、
  Tree-sitter语法和新增调用门禁。阶段2适配与严格阈值只接受离线逻辑测试，
  正式 CLI 和实验不执行直通；正式23仓合成与可选 Clang 模板验证尚未运行。
...
独立发现仓库内部成立的习语。不同仓库的候选、embedding 和 DBSCAN 输入不得混合；
...
发送 `repos/` 或筛选前26仓源码。第一轮阶段3处理5个簇，10个 Agent 结构化调用
...
不是正式双人盲审、模型质量结论或论文结果。正式23仓习语判断、合成、人工审计
...
### 2026-07-28 Sol 6.0 阶段3/4理由链与习语开放分类

阶段3与阶段4 Schema 升至v7。新增 `cpp-common-idioms-v1` 受控目录，依据 thesis
全局文献 `[E003]`、`[E025]`、Coplien 专著 `[E111]`、C++ Core Guidelines
`[E112]`、cppreference 的 RAII `[E113]` 与 Rule of Three/Five/Zero `[E114]`
以及 Boost.Scope Scope Guards `[E115]`，收录26个可从当前证据边界相对可靠
识别的常见类型。
最终有效性 Agent 显式输出 `is_idiom`、非空判断理由和结构化类型：可精确对应
目录时为 `cataloged`，无法可靠对应但仍通过全部习语门禁时为
`repository_specific`/`仓库特有习语`，不是习语或技术失败时为
`not_applicable`。确定性标准化拒绝未知编号、类型矛盾、超过3个类型、空理由、
越界或非有限置信度。类型不进入业务评分、异味风险或 DBSCAN 参数选择。

语义、异味、规划、组装和质量 Agent 均必须返回非空中文理由。阶段3记录
`idiom_classification`、`agent_reasons` 与汇总后的 `decision_reason`；阶段4
把来源阶段3理由和类型传给规划、质量和异味复审，对当前合成结果重新判断和分类，
并以顶层 `source_judgments` 保存来源审计证据，同时记录规划、组装、质量、类型
与异味理由。缺少理由或分类合同无效时按 `invalid_domain_payload` 安全拒绝或
停止，不把技术失败伪装成有效业务结论。

真实 smoke 只使用手写合成 C++，模型为 `gpt-5.6-luna`，没有发送 `repos/` 或
26仓源码。受限网络中的首次4簇运行全部发生 `APIConnectionError`；每个 Agent
完成两次有界尝试，4簇均按安全合同拒绝，严格上下文4/4验证成功，
`technical_failure_count=4`。获准网络后的类型覆盖轮处理4簇、8个 Agent 调用，
均一次成功：接受 `raii`、`erase-remove` 和1个 `仓库特有习语`，拒绝普通计数器
递增并标为 `非习语`；三个语义/类型/异味理由字段在每条记录中均非空。该轮使用
20,634个输入 token 与2,463个输出 token。

第二轮首先复用旧的“获取/检查 + 消费/手工关闭”夹具。4个 Agent 调用均一次
成功，但阶段3拒绝2/2：前者明确判为非习语，后者虽判为仓库特有习语，但业务分
67.71未达到70。该结果证明新增 `is_idiom` 门禁不会为了进入阶段4而放宽单簇
标准；没有据此修改提示词、算法或阈值。该边界轮使用10,333个输入 token 与
2,040个输出 token。

随后改用两个各自完整且共享同一严格区域的合成候选，而不是修改业务标准。
阶段3接受2/2，分别为 `raii` 与 `仓库特有习语`，使用10,481个输入 token 与
1,406个输出 token。阶段4两次运行均接受1/1个 `synthesis_delta`：第一次验证
规划、组装、质量/类型与异味理由链，使用10,174个输入 token 与1,693个输出
token；发现来源理由只能从嵌套异味输入读取后，新增顶层 `source_judgments`，
第二次使用10,196个输入 token 与2,375个输出 token，确认两条来源理由和类型均
可直接审计，当前合成结果独立重判为 `raii`。两个阶段4运行的4个 Agent 均一次
成功，所有五类理由非空。

五次获准网络运行合计24个有效结构化响应，没有 JSON 修复、逻辑重试或技术失败；
总计61,818个输入 token 与9,977个输出 token。中转服务未提供精确账单，因此不
声称具体费用。这些结果只验证连通性、Schema、分类分支、理由传播和安全边界，
不构成模型质量结论、类型目录完备性证据或正式研究结果。输入与产物位于已忽略的
`outputs/llm-smoke/stage34-20260728-*` 和
`results/llm-smoke/stage34-20260728-*`。

离线验证最终通过 `pip check`、`compileall`、全部92项 `unittest` 和
`git diff --check`。定向阶段3/4与提示词合同测试共34项，覆盖通用/特有分类、
`is_idiom` 硬否决、非有限置信度、理由字段、阶段3到阶段4传播、顶层
`source_judgments` 和汇总分类计数。

本轮进一步明确了 Schema v7 分类在最终习语库中的知识组织语义，但没有修改代码、
提示词、Schema、目录内容或阈值：`cataloged` 在知识层称为“目录化通用习语”，
按 `taxonomy_version + catalog_id` 支持各仓库独立挖掘完成后的跨仓库汇总与
复用；`repository_specific` 在知识层称为“仓库专属习语”，保留项目和阶段内
记录作用域，并收纳当前目录尚未总结的不常见通用习语；二者并集称为“全量联合
...
默认模型为 `gpt-5.6-luna`。中转服务没有可核验的精确价格；该段记录的是筛选前
26仓历史估算，当前23仓运行必须重新确认预算。输入均来自公开仓库，但源码片段仍
会发送到配置端点。
...
### 2026-07-29 阶段2聚类质量筛选与23仓正式主数据集

在冻结的26仓 DBSCAN 产物上复核逐仓聚类指标、代表代码、簇成员源码与来源路径
后，将 `cpp-httplib`、`entt` 和 `simdjson` 标记为“阶段2后排除”。三个仓库不再
进入阶段3/4、论文主结果或正式 baseline 全仓运行，但固定源码与阶段1/2产物继续
保留为筛选前证据。GSL 仍是本地不保留的“淘汰”项目。此次修改没有重新运行
Parser、Embedding、DBSCAN 或付费 LLM，也没有删除历史产物。

排除依据分别为：cpp-httplib 的588个簇中582个为单文件簇，跨文件簇仅6个；
entt 的942个簇中719个源码完全重复，79.0%的簇内成员来自 `single_include/`；
simdjson 的1,031个簇中956个源码完全重复，83.9%的簇内成员来自
`singleheader/`。cpp-httplib 仍有真实网络与资源管理语义，只是不满足主数据集
的跨文件复现测量条件；不得把三者统一描述为“没有习语”。

清单重算后的阶段1正式统计为23仓、7,505个源码文件、1,013,071行有效代码、
72,090个函数、203,609个候选和91.09%可靠 AST 字节覆盖率。阶段2筛选前后主要
变化为：

| 指标 | 26仓 | 23仓 |
|---|---:|---:|
| model-ready 候选 | 233,912 | 200,047 |
| 有效簇 | 31,965 | 29,404 |
| 微平均覆盖率 | 66.878% | 66.594% |
| 全部簇源码完全重复率 | 18.50% | 14.06% |
| Top100 源码完全重复率 | 12.78% | 7.85% |
| Top100 强结构代理率 | 40.12% | 43.27% |
| `Top100 ∩ Top20%` 强结构代理率 | 43.97% | 48.44% |
| Top100 代表 AST 子树均值 | 68.27 | 70.98 |

完整筛选语义、全部前后指标和当前23仓名单记录在
`docs/research/cpp-clustering-quality-screening.md`。机器可读状态由
`cpp-dataset-selection.json` 驱动；manifest 校验器会校验23个正式仓库和3个
历史仓库的 commit 与 Parser 产物，同时确认 GSL 本地路径不存在。

本轮重新生成 `cpp-dataset-manifest.json` 和 `cpp-dataset-statistics.json`，
清单交叉校验结果为23个正式项目、3个历史项目、1个淘汰项目、0个错误；同时通过
`pip check`、`src scripts` 的 `compileall`、全部95项确定性离线测试和
`git diff --check`。未修改共享基础设施，不需要执行双仓同步检查。

### 2026-07-29 阶段2到阶段3的保守簇归并与输入精简

新增 `src.mining.cluster_merge`，逐仓读取冻结的
`clustering-final/clusters.pkl` 与既有 `embeddings.pkl`，把派生结果写入新的
`stage2/clustering-merged/`，未重新运行或覆盖 Parser、Embedding、DBSCAN 和
真实 LLM。`cpp-httplib`、`entt`、`simdjson` 未进入运行，也未生成派生目录。

中心代码按 C++ 词法 token 等价时直接归并；模糊归并固定要求 AST 节点序列与
token 数相同、序列相似度至少0.92，并且所有差异都只能是已声明局部变量的一致
一一换名。对早期较宽实现的逐组语义复核发现，单纯依靠 AST 与语义签名仍可能把
不同标签参数、事件常量、回调目标和成员字段合并，因此正式实现收紧到当前规则。
归并后使用全部成员原始 embedding 的算术均值重算质心，再按余弦距离选择最近的
真实成员。七列簇表、完整成员与 `infos` 保持不变；来源 label、归并理由和输入
SHA-256 保存在顶层元数据。
...
|---|---:|---:|
| 有效簇数 | 29,404 | 29,401 |
| 簇内成员数 | 133,220 | 133,220 |
| C++词法纯重复率 | 14.2056% | 14.1934% |
| 强结构代理率 | 26.1427% | 26.1522% |

实际只归并3组：`drogon` 的1组是同一声明中局部结果变量换名；`envoy` 的2组是
仅注释和排版不同的词法等价覆盖函数。其余21仓不合并。完整语义复核和指标定义见
`docs/research/cpp-cluster-merge-report.md`。

阶段3 Schema 升至v8。完整 `member_codes` 和 `source_infos` 继续支持本地规则、
支持度、审计、评价与实例归类；语义和异味 Agent 的源码证据只含离新质心最近的
真实代表、按 C++ 词法 token 去重后的其他变体，并附原始成员数、变体数、文件数
和源码位置数。经路径、范围和哈希验证的代表上下文仍执行严格零调用门禁，但不再
进入阶段3提示词。相关离线测试覆盖词法语义保留、两类归并、语义关键差异拒绝、
成员无损、重算代表、LLM 输入精简和上下文不外发。本轮通过 `pip check`、
`src tests` 的 `compileall`、全部98项确定性离线测试、归并 CLI `--help`、
23仓输入哈希/成员集合/来源 label/七列 Schema 校验和 `git diff --check`。

### 2026-07-29 正式数据集分类与分层实验口径

正式23仓新增两条只用于结果汇总的分类轴。互斥主领域按固定commit的核心生产
用途划分为：数据表示与泛型基础库6仓、开发工具与工程基础设施4仓、网络与
分布式系统5仓、应用与跨平台界面5仓、存储与并发基础设施3仓。跨领域信息继续
由 `project_type` 和 `idiom_focus` 保存。

相对分析复杂度由阶段1的有效代码行、核心文件和候选总数计算：三项指标分别转为
正式23仓内的升序平均秩百分位，等权平均后近似等频分为低8、中8、高7。该值只
表示当前流水线的处理负荷，不是代码质量、圈复杂度或维护难度，也不进入筛选、
DBSCAN、LLM或合成配置。

`cpp-dataset-selection.json` 保存分类政策和人工主领域，
`cpp-dataset-manifest.json` 保存逐仓原始指标、百分位、综合分和复杂度层，
`cpp-dataset-statistics.json` 保存领域、复杂度与交叉分布。
`scripts.analyze_cpp_dataset` 负责复算并在 `validate-manifest` 中检查逐仓分类、
汇总和正式范围一致性。选择方法明确为有约束的目的性最大差异抽样，仍要求报告
全部23仓逐仓结果和仓库宏平均；领域与复杂度分层只作预先记录的附加分析。

完整口径、逐仓分类、选择漏斗和后续实验建议见
`docs/research/cpp-dataset-classification.md`。新增4项纯离线分类单元测试；完整
离线套件共102项并全部通过。清单交叉校验继续得到23个正式项目、3个历史项目、
1个淘汰项目和0个错误，同时通过 `pip check`、`compileall`、全部文档 JSON
解析和 `git diff --check`。

### 2026-07-29 Sol 6.2 正式23仓、簇归并与分层数据合同

项目版本升级到 `Sol 6.2`。本次发布统一收口了三个连续任务：

1. 正式论文数据集由阶段1/2的26仓筛选为23仓；`cpp-httplib`、`entt` 和
   `simdjson` 保留固定源码与历史证据，但不进入真实LLM、合成或主结果。筛选同时
   使用纯重复率、跨文件支持、强结构代理与具体簇语义，不按单一指标机械淘汰。
2. 冻结DBSCAN之后增加仓库内保守簇归并；只接受C++词法等价或AST同构且全部差异
   为已声明局部变量一致换名的簇，保留完整成员和来源信息，并用全部embedding
   重算质心与真实代表。23仓有效簇由29,404降至29,401，成员数保持133,220。
3. 阶段3的规则与审计仍使用完整簇，LLM只接收真实代表、词法去重变体及成员数、
   变体数、文件数和源码位置数；代表上下文继续执行本地严格门禁，但不再进入
   阶段3提示词。判断产物Schema升至v8。
4. 正式23仓增加主领域与相对分析复杂度分类，机器可读清单、统计、论文实验设计
   和选择合理性说明同步更新；分类只用于逐仓结果完成后的分层汇总，不参与参数
   路由或项目质量判断。

本版本没有重新运行或覆盖Parser、Embedding、DBSCAN与真实LLM冻结产物。新增
派生簇产物写入独立忽略路径，版本控制只提交实现、测试、结构化清单和研究文档。

### 2026-07-30 Sol 6.3 领域流水线精简重构

项目版本升级到 `Sol 6.3`。本轮在不修改提示词、Schema、评分阈值和算法合同的
前提下完成以下核心重构：

1. 阶段3/4 Runtime 成为初始化状态的唯一事实来源，删除与 Runtime 重复维护的
   `_initialized` 状态；流水线关闭时只关闭内部创建的模型客户端，外部注入客户端
   的所有权仍由调用方持有。
2. Agent 路由异常隔离、调用轨迹和未执行轨迹集中到现有 Agent 基础设施。普通
   Runtime 路由异常继续按领域合同安全拒绝当前簇或候选组；取消与进程中断不再被
   普通失败回退吞掉。
3. `JsonLLMAgent` 删除未被调用方使用的重试、延迟和超时配置层，直接执行项目已经
   冻结的120秒超时、两次逻辑尝试和单次 JSON 修复合同。JSON 解析、Schema 校验、
   单次修复和最终失败路径保持不变。
4. 已通过 JSON Schema 的响应字段不再重复执行类型猜测和默认值拼装，只保留
   Schema 未覆盖的领域校验：分数有限且位于0–100、判断理由非空、习语分类合同
   一致、规划索引属于输入，以及异味类别、置信度和证据字段有效。
5. 阶段4结果数据类为提前退出路径提供明确空值默认，删除各门禁分支中重复的空
   `plan`、`review`、`smell`、`scorecard` 等字段装配；产出的字段和 Schema
   保持不变。
6. 阶段4候选上限小于2时直接报告配置错误，不再静默提升为2。Parser、聚类归并、
   阶段3/4 CLI 和数据集审计中的文件摘要统一使用 Python 3.12
   `hashlib.file_digest`，不改变 SHA-256 结果。

生产与脚本代码净减少116行。新增3项离线测试覆盖非法候选上限、路由失败回退和
取消传播；完整105项离线测试、`pip check`、`src/scripts/tests` 编译检查、
23仓数据清单校验、17文件跨项目共享基础设施检查、相关导入与 CLI 帮助入口和
`git diff --check` 均通过。验证未下载模型、未发送源码，也未产生付费 LLM 请求。

### 2026-07-30 Sol 6.4 阶段4完整成员共现分组

项目版本升级到 `Sol 6.4`。阶段4不再只使用聚类中心对应的代表位置决定合成
资格，而是遍历每个阶段3已接受习语的完整 `source_infos`，按成员
`project + file + function_extent` 建立区域倒排索引。至少两个不同习语簇在
同一区域各有成员时才形成候选组；同一簇在一个区域内的重复成员会去重，但该簇
可以参与多个真实共现区域。候选自身 `extent` 与 `start_byte/end_byte` 只用于
当前区域局部代码和稳定源码顺序，不要求不同习语片段的字节区间相交。

规划、组装和质量复审现在同时接收簇级模板与当前区域
`matched_occurrences`；组装优先使用局部真实代码绑定模板。上下文门禁校验组内
全部匹配成员的区域身份和整文件 SHA-256，再读取共同函数/区域。阶段4 Schema
升至v8，新增 `matched_source_infos`、`matched_occurrences`、
`region_identity`、`source_order_candidate_ids` 和 `cooccurrence_evidence`；
原 `source_infos` 继续表示所选簇的完整支持证据。运行汇总新增
`region_candidate_membership_count`，用于区分输入习语数量与跨区域候选成员数。
新增3项离线测试覆盖“聚类中心不同但非中心成员同区域”、从该非中心成员区域
加载并验证上下文，以及同一习语参与多个共现区域；既有分组测试改为同时验证
局部代码和源码顺序，另保留同簇重复成员不成组、历史位置标签不成组、上下文/
哈希零调用拒绝和候选上限测试。本轮通过 `.venv/bin/python -m pip check`、
`src/tests` 全量 `compileall` 和全部108项
确定性离线测试；验证未下载模型、未发送源码，也未产生付费 LLM 请求。未修改
共享基础设施文件，因此不需要执行双仓同步检查。

### 2026-07-30 阶段4组合搜索设计决策

已确认规划 Agent 的设置目的，是避免对同一成员共现区域内的候选执行指数级全
子集枚举；如果对每个数学子集都运行组装、质量复审和异味审查，调用量无法接受。
后续所称“所有可能组合”限定为显式计划上限内所有具有明确数据、控制、生命周期、
异常处理或稳定顺序关系且值得尝试的组合，不包括缺乏语义联系的任意子集。

多计划优化选择“每区域一次批量返回”：规划 Agent 一次审查完整候选组并返回
`plans`，编排层规范化索引、建立稳定组合键、确定性去重，再对每个合法计划独立
组装和复审。不采用“每次返回一组并循环到模型自行停止”的方案，因为它增加规划
调用、容易重复或过早停止，也无法给出稳定的成本和覆盖边界。

本次只冻结并同步上述设计文档，没有修改提示词、消息 Schema、流水线或 artifact
Schema。当前 Sol 6.4 源码仍然是每区域一次规划并只返回一个
`selected_indices`；批量 `plans`、显式 `max_plans_per_region`、计划级
checkpoint、产物表达和相应离线测试仍待后续实现。因未修改共享基础设施文件，
不需要执行双仓同步检查。

### 2026-08-01 Sol 6.4 阶段4区域级批量多计划

本次继续完善 `Sol 6.4`。规划 Agent 的响应由单个
`should_synthesize + selected_indices` 改为顶层非空总 `reason` 和有界
`plans` 数组；每个计划必须分别提供 `selected_indices`、`relation_kind`、
`synthesis_goal`、`ordering_constraints`、`expected_improvement` 和非空
`reason`。Agent 每个成员共现区域只调用一次，同时审查区域内全部候选；找不到
有效关系时返回空数组和非空总理由。默认 `max_plans_per_region=8`，该值同时进入
动态 JSON Schema、请求提示、运行合同、CLI 和 checkpoint 元数据；响应超限时
不截断也不执行。规划提示明确排除没有数据、控制、生命周期、异常处理或稳定源码
顺序关系的数学子集。

编排层对每项索引先去重和排序，再校验范围、至少两个不同候选及所有非空计划
字段；完全相同的规范化候选集合只保留首项，不同组合允许共享候选。稳定
`combination_key` 由成员共现区域键与排序后的候选编号确定性哈希得到。每个合法
且唯一计划分别组装，并在确定性门禁通过后独立执行质量/类型复审和异味审查；
单项组装或未预料编排失败只拒绝该计划，不取消同一区域其他计划。规划循环完成后
直接停止，不再次询问规划 Agent。CLI 将同一区域返回的多个 `SynthesisResult`
展开为独立记录，SQLite checkpoint 仍以区域为提交单位并把
`max_plans_per_region` 纳入续跑一致性校验。

阶段4 artifact 升至 Schema v9。每条计划结果以 `region_planning` 保存区域级总
理由、调用状态、显式上限与非法/重复计划摘要，以 `combination_key` 和单项
`synthesis_plan` 保存规范化执行身份；汇总新增 `planning_call_count`、
`valid_unique_plan_count` 和 `rejected_planning_plan_count`。新增4项确定性离线
测试覆盖有界 `plans` Schema、索引排序/去重/越界/单候选/空字段拒绝、共享候选
多计划逐项执行及失败隔离，以及空计划带理由停止。`.venv/bin/python -m pip check`、
`src/tests` 全量 `compileall`、阶段4 CLI 帮助入口、`git diff --check` 和全部112项
离线测试均通过；未下载模型、未发送源码，也未产生付费 LLM 请求。未修改共享
基础设施文件，因此不需要执行双仓同步检查。

随后在明确授权下使用完全合成的 C++ 函数执行一次有界真实 smoke，没有发送
`repos/` 或其他仓库源码。模型为 `gpt-5.6-luna`，输入固定为1个成员共现区域、
4个阶段3已接受候选和 `max_plans_per_region=2`。规划 Agent 只调用1次且一次成功，
审查全部候选后返回2个计划：`[0, 1]` 为资源获取/释放生命周期配对，`[2, 3]`
为互斥锁获取/释放控制关系；没有返回缺乏独立语义关系的跨组子集。两项计划的
全部必填说明字段均非空，索引已规范化，稳定组合键互不重复，编排层分别完成2次
组装、质量/类型复审和异味审查，所有 Agent 均一次成功。锁计划以质量分76接受；
资源计划因只是直接拼接、未形成明确质量提升而以质量分42拒绝，说明多计划执行
不会绕过逐计划独立门禁。汇总为规划调用1次、合法唯一计划2项、合成尝试2项、
接受1项、拒绝1项、技术失败0项；实际用量为16,892个输入 token 和3,553个输出
token。产物、SQLite checkpoint、无源码验证摘要、完整命令、运行库版本及首次
受限网络 `APIConnectionError` 记录位于被忽略的
`outputs/llm-smoke/stage4-multiplan-20260801/` 与
`results/llm-smoke/stage4-multiplan-20260801/`。该 smoke 只证明 Schema v9 和
区域级批量多计划编排生效，不作为23仓质量、成功率或阈值校准证据。

## 当前阻碍与延期工作

1. 中转端点、凭据加载、Luna JSON 模式以及当前二态评分、自动上下文、共享异味
   和失败回退合同已通过22个初始纯合成真实响应、Sol 5.9的16个收口响应、本轮
   Schema v7的24个历史有效结构化响应与离线故障注入。当前阶段3为 Schema v8，
   阶段4为 Schema v9；完整语料的付费
   Agent 评估仍有意延期，因为逐仓预算、非公开源码披露范围和研究有效性需要
   单独冻结；合成 smoke 的成功率不能外推为23仓质量。
2. 筛选前 26 项目已完成阶段1、全量 tokenizer 长度治理、逐仓
   UniXcoder embedding、DBSCAN 默认基线、HDBSCAN 对照和单一 DBSCAN 最终
   聚类；当前正式主数据集为其中23仓，三个“阶段2后排除”仓库只保留历史证据。
   更早的
   三个快照上的 96,039 个片段仍只作旧语料证据，不得与新正式产物混用。
3. Baseline 实现和有界验证已获授权并完成，IdioMine-CPP 的筛选前26仓判断前
   候选统计与调用估算继续作为历史证据。正式23仓真实判断/合成、完整四条
   baseline 的全仓正式重复和
   CIMAS token 测量及源码披露/成本决策属于后续付费 Agent 运行门槛；后续应先由
   习语判断逐仓消费 `clustering-merged/clusters.pkl`，
...
IdioMine-CPP 的26仓1,205次逻辑调用、2,410次端点请求和约1.77M主请求 token
上界已经失效；下一步应先按当前23仓清单重新执行 `--estimate-only`。CIMAS-CPP
主线也应先对23仓运行习语判断的离线规则预检，统计硬拒绝、
